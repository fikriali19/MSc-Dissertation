...
for (h in 1:k) {
  for (t in 1:T) {
    term_eff_energy[h,t] = 1 / 
      (1+(1/n)*
         (sum (w[1:n]/
                 as.numeric(df_energy[h,((t-1)*(m+n+1)+(m+1)):
                                        ((t-1)*(m+n+1)+(m+n))])
...
rownames(weights_slacks) <- rownames(df_energy)
colnames(weights_slacks) <- c(rep("lambda",k*T),rep("s_i",m*T),rep("s_j",n*T),
                              rep("s_co",T))
weights_nrg = weights_slacks[,1:(k*T)]
slacks_nrg = weights_slacks[,(k*T+1):vars]

# projections
proj_data_nrg = matrix(,nrow = nrow(df_energy), ncol = ncol(df_energy))
rownames(proj_data_nrg) = rownames(df_energy)
colnames(proj_data_nrg) = colnames(df_energy)

for (h in 1:k){
  for (t in 1:T){
    for(i in 1:m){
      proj_data_nrg[h,((t-1)*(m+n+B))+i] = 
        as.numeric(df_energy[h,((t-1)*(m+n+B))+i]) - slacks_nrg[h,((t-1)*m)+i]
    }
    for(j in 1:n){
      proj_data_nrg[h,((t-1)*(m+n+B))+(m+j)] = 
        as.numeric(df_energy[h,((t-1)*(m+n+B))+(m+j)]) 
      + slacks_nrg[h,(T*m)+((t-1)*n)+j]
    }
    for(b in 1:B){
      proj_data_nrg[h,((t-1)*(m+n+B))+(m+n+b)] = 
        as.numeric(df_energy[h,((t-1)*(m+n+B))+(m+n+b)]) 
      - slacks_nrg[h,(T*m)+(T*n)+((t-1)*B)+b]
    }
  }
}

# FEI values
FEI_nrg = matrix(,nrow = nrow(df_energy), ncol = ncol(df_energy))
rownames(FEI_nrg) = rownames(df_energy)
colnames(FEI_nrg) = colnames(df_energy)
for(h in 1:k){
  for(i in 1:(T*(m+n+B))){
    FEI_nrg[h,i] = as.numeric(df_energy[h,i])/as.numeric(proj_data_nrg[h,i]) - 1
  }
}
